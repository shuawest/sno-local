# Ansible Role - lab_collections.aws_route53_subdomain.route53_owner

Role for creating Route53 user with limited access to control a subdomain as the AWS hosted zone owner.


### Reference Documentation 
* https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/specifying-rrset-conditions.html
* https://docs.aws.amazon.com/IAM/latest/UserGuide/id.html
* https://cert-manager.io/docs/configuration/acme/dns01/route53/
* https://docs.ansible.com/ansible/latest/collections/community/aws/route53_module.html
* https://docs.ansible.com/ansible/latest/collections/community/aws/iam_policy_module.html 
* https://docs.ansible.com/ansible/latest/collections/community/aws/iam_group_module.html
* https://docs.ansible.com/ansible/latest/collections/community/aws/iam_user_module.html
* https://galaxy.ansible.com/dseeley/ansible_vault_pipe 